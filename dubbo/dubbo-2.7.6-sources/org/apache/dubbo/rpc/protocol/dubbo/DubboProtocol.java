...
public class DubboProtocol extends AbstractProtocol {
    ...
    private ExchangeHandler requestHandler = new ExchangeHandlerAdapter() {
        ...
        public void received(Channel channel, Object message) throws RemotingException {
            if (message instanceof Invocation) { // 对象为Invocation表示远程是调用方,当前是服务提供方
    ...
    private ProtocolServer createServer(URL url) {
        url = URLBuilder.from(url)
                ...
                // enable heartbeat by default 心跳间隔 默认60秒一次
                .addParameterIfAbsent(HEARTBEAT_KEY, String.valueOf(DEFAULT_HEARTBEAT))
                .addParameter(CODEC_KEY, DubboCodec.NAME) // 编解码协议
                .build();
    ...
    private ExchangeClient initClient(URL url) {
        ...
        try {
            // onnection should be clazy
            if (url.getParameter(LAZY_CONNECT_KEY, false)) {
                client = new LazyConnectExchangeClient(url, requestHandler);

            } else { // 此时requestHandler在客户端用于接收提供方返回的信息,而且并不是走received接口,而是由包装类HeaderExchangeHandler#received来处理
