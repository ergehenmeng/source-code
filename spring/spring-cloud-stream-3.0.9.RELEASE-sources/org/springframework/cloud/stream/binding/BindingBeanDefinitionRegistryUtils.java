...
public abstract class BindingBeanDefinitionRegistryUtils {
    // qualifierValue = @Input中value的值, name=value的值或方法名
	...
	private static void registerBindingTargetBeanDefinition(
			...
			String bindingTargetInterfaceBeanName, // 接口名称
			String bindingTargetInterfaceMethodName, BeanDefinitionRegistry registry) {
		...
		rootBeanDefinition.setUniqueFactoryMethodName(bindingTargetInterfaceMethodName); // @Input @Output标注的方法
		rootBeanDefinition // 一个接口可能有多个包含@Input @Output的方法, 此处用来区别不同方法的
	...
	public static void registerBindingTargetsQualifiedBeanDefinitions(Class<?> parent,
		...
		if (type.isInterface()) { // 将接口注册为一个Bean
			RootBeanDefinition rootBeanDefinition = new RootBeanDefinition(
    ...
    // 如果注解包含value属性,则使用value属性的值,否则使用方法名
