...
public abstract class SingleThreadEventExecutor extends AbstractScheduledEventExecutor implements OrderedEventExecutor {
    ...
    protected boolean runAllTasks(long timeoutNanos) {
        ...
        if (task == null) {
            afterRunningAllTasks(); // 任务处理完了,处理tailTask任务
            return false;
        }
        ...
        for (;;) { // 此方法用于执行 NioEventLoop 队列中的任务方法
            safeExecute(task);

            runTasks ++;
            // 每64个任务后检查一下,如果执行时间已用完,则中断执行
    ...
    // 将任务添加的队列中,开启线程优先执行 io事件,再执行任务队列
