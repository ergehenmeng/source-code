...
public class FastThreadLocal<V> {
    // 待移除的下标, 主要用于清除线程变量,默认值 0,  即每个线程中ThreadLocalMap数组第一个元素就是用来存放待移除的set集合
    ...
    public static void removeAll() {
        ...
        try {// 当前线程中绑定的变量对象(多个FastThreadLocal)全部清空
    ...
    // FastThreadLocal在InternalThreadLocalMap数组中存放线程变量的下标位置, 从1开始
    ...
    /**
     * 设置值后 将FastThreadLocal添加到待销毁的列表中,在线程销毁时移除变量
     * @return see {@link InternalThreadLocalMap#setIndexedVariable(int, Object)}.
     */
    private void setKnownNotUnset(InternalThreadLocalMap threadLocalMap, V value) {
        if (threadLocalMap.setIndexedVariable(index, value)) { // true表示第一次设置,false:不是第一次,因此不需要重复加入待移除队列
            addToVariablesToRemove(threadLocalMap, this);
        }
    }
    ...
    public final void remove(InternalThreadLocalMap threadLocalMap) {
        ...
        // 移除当前ThreadLocal对象, 同时将该对象在LocalMap从待移除集合中移除
