...
public abstract class HttpObjectDecoder extends ByteToMessageDecoder {
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf buffer, List<Object> out) throws Exception {
        ...
        case SKIP_CONTROL_CHARS:
            // Fall-through
        case READ_INITIAL: try { // http请求 首行
        ...
        if (line.length() > 0) {
            do {
                ...
                if (name != null && (firstChar == ' ' || firstChar == '\t')) {
                    // please do not make one line from below code as it breaks
                    // +XX:OptimizeStringConcat optimization
                    String trimmedLine = line.toString().trim();
                    String valueStr = String.valueOf(value);
                    value = valueStr + ' ' + trimmedLine;
                } else {
                    if (name != null) { // 上一行头信息的数据保存完,再解析当前行的数据
    ...
    // 返回结果 [GET, http://www.baidu.com/index.html, HTTP/1.1]
    ...
    private static class HeaderParser implements ByteProcessor {
        ...
        public AppendableCharSequence parse(ByteBuf buffer) {
            ...
            seq.reset();
            int i = buffer.forEachByte(this); // 尝试读取一行数据,并将读到的数据放入到seq中. -1:表示读完了也没读到换行符
            if (i == -1) {
                size = oldSize;
                return null;
            }
