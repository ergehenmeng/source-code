...
public class IdleStateHandler extends ChannelDuplexHandler {
    ...
    private final long readerIdleTimeNanos; // 读超时时间 纳秒
    private final long writerIdleTimeNanos; // 写超时时间 纳秒
    private final long allIdleTimeNanos; // 读写超时时间 纳秒
    ...
    private final class ReaderIdleTimeoutTask extends AbstractIdleTask {
        ...
        protected void run(ChannelHandlerContext ctx) {
            ...
            } // 当前正在执行读操作,则下次检测时间就是标准的readerIdleTimeNanos时间

            if (nextDelay <= 0) { // 上次读与当前时间间隔大于标准超时时间, 需要触发一个读超时事件,同时指定下次检测的时间
            ...
            } else {// 在没有超时前发生读事件 1:当前在读操作, 2:刚刚进行了读操作, 此处不太明白设置一个更短的超时时间的作用???
