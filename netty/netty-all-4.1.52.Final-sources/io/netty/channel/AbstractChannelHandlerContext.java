...
abstract class AbstractChannelHandlerContext implements ChannelHandlerContext, ResourceLeakHint {
    ...
    static void invokeChannelRead(final AbstractChannelHandlerContext next, Object msg) {
        ...
        if (executor.inEventLoop()) {
            ...
            executor.execute(new Runnable() { // 实际上就是将任务放到任务队列中,执行的时间不确定(第一次时会启动一个线程)
                @Override
                public void run() {
                    next.invokeChannelRead(m);
                }
    ...
    private AbstractChannelHandlerContext findContextInbound(int mask) {
        ...
        do {
            ctx = ctx.next;// 查找后一个handler,如果符合要求就直接返回
    ...
    private AbstractChannelHandlerContext findContextOutbound(int mask) {
        ...
        EventExecutor currentExecutor = executor();
        do {
            ctx = ctx.prev;// 查找前一个handler,如果符合要求就直接返回
        } while (skipContext(ctx, currentExecutor, mask, MASK_ONLY_OUTBOUND));
        return ctx;
    }
    // 如果mark=MASK_EXCPETION_CAUGHT InboundHandler与OutboundHandler均包含该方法,如果因为InboundHandler导致的异常,可能会走OutboundHandler,从而影响异常处理逻辑
    private static boolean skipContext(
            AbstractChannelHandlerContext ctx, EventExecutor currentExecutor, int mask, int onlyMask) {
        // Ensure we correctly handle MASK_EXCEPTION_CAUGHT which is not included in the MASK_EXCEPTION_CAUGHT
        return (ctx.executionMask & (onlyMask | mask)) == 0 || // 表示该类中没有指定的方法(或者该方法上有@Skip注解), 需要跳过
    ...
    // channel未注册,或者handler还未添加到pipeline中,均会被设置为pending中的状态
     ...
     * true: 当前handler已经调用过handlerAdded, false: 当前handler还未调用handleAdded ???
