...
public abstract class AbstractChannel extends DefaultAttributeMap implements Channel {
    ...
    /**
     * Returns a new {@link DefaultChannelPipeline} instance. 相当于channel与pipeline进行相关关联
     */
    protected DefaultChannelPipeline newChannelPipeline() {
        return new DefaultChannelPipeline(this); // 客户端NioSocketChannel, 服务端 NioServerSocketChannel
    }
    ...
    protected abstract class AbstractUnsafe implements Unsafe {
        ...
        // 服务端绑定注册时, promise是一个新创建的ChannelPromise对象
        @Override
        public final void register(EventLoop eventLoop, final ChannelPromise promise) {
            ...
            // 从EventLoopGroup(BossGroup)中选择一个并关联上NioServerSocketChannel或者NioSocketChannel
            AbstractChannel.this.eventLoop = eventLoop;

            if (eventLoop.inEventLoop()) {
                register0(promise);
            } else {
                try { // 先启动eventLoop上的线程, 进行轮询针对不同的事件做不同的处理,只有符合一定条件才会调用register0进行注册操作
        ...
        private void register0(ChannelPromise promise) {
            try {
                ...
                AbstractChannel.this.doRegister(); // 注册感兴趣事件 调用java原生方法
                neverRegistered = false;
                registered = true;
                // 在服务端启动时添加了一个ChannelInitializer,而该handler是作为回调方式执行,因此invokeHandlerAddedIfNeeded会触发ChannelInitializer#handlerAdded,目的用于添加ServerBootstrapAcceptor,
                // 而在客户端接入后依旧会触发该接口,目的主要添加一些自定义的handler,注意如果是ChannelInitializer方法,则会在执行完毕之后从执行链中干掉
                ...
                if (isActive()) {
                    if (firstRegistration) { // 客户端链接进来时触发fireChannelActive事件
        ...
        public final void bind(final SocketAddress localAddress, final ChannelPromise promise) {
            ...
            if (!wasActive && isActive()) {
                invokeLater(new Runnable() {
                    @Override
                    public void run() {
                        pipeline.fireChannelActive(); // 服务端启动时也会触发该事件
                    }
                });
            }
        ...
        // 实际上就是将已经flushed数据写入到socket中 channel.writeAndFlush = (1.包装成Entry添加到链表尾部, 2.将其从unflush区域转到flunshed区域, 3.写入到socket)
