...
public final class ChannelOutboundBuffer {
    ...
    // 整个数据结构是单链表, 把该链表分成两个区间 flush区间[flushedEntry, unflushedEntry), 非flush区间 [unflushedEntry, tailEntry]
    // (            待写入socket的数据         )(         待执行flush的数据       )
    // Entry(flushedEntry) --> ... Entry(unflushedEntry) --> ... Entry(tailEntry)
    // The Entry that is the first in the linked-list structure that was flushed. 即将要写入socket的起点位置
    private Entry flushedEntry;
    // The Entry which is the first unflushed in the linked-list structure 被添加的开始节点，但没有准备好被消费
    private Entry unflushedEntry;
    // The Entry which represents the tail of the buffer 最后一个节点
    private Entry tailEntry;
    // The number of flushed entries that are not written yet 待写入的entry的个数
    private int flushed;
     ...
     *  A -- B -- C(tail)  unflushedEntry = A 调用addMessage 在非flush区间的尾部添加Entry
     */
    public void addMessage(Object msg, int size, ChannelPromise promise) {
        ...
        // addFluash后再调用addMessage添加D E 则变为 A -- B -- C -- D -- E(tail)  flushedEntry = A   unflushedEntry = D
     ...
     * 把非flush区间的[unflushedEntry, tailEntry]的entry逐一添加到flush区间
     */
    public void addFlush() {
        ...
        // A -- B -- C(tail)  unflushedEntry = A  flushedEntry = null
        // A -- B -- C(tail)  flushedEntry = A   unflushedEntry = null
